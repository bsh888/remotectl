...
与向日葵、TeamViewer 等商业工具属于同一 WebRTC 技术栈，核心差异在于**体验深度**与**自主可控**：
...
| 浏览器直接控制（无需安装客户端） | ✓ | 部分 |
| 码率 / 分辨率 / 帧率完全自定义 | ✓ | 部分 |
| 移动端完整键盘（修饰键 + 数字行 + F1–F12） | ✓ | 部分 |
| 会话内聊天 + 文件传输（P2P 直连） | ✓ | — |
| 完全免费 | ✓ | 部分 |

- **自托管**：部署一台公网 Linux 服务器即可，视频流走 WebRTC P2P，服务器仅转发信令，不经手任何音视频数据。
- **画质自定义**：`bitrate`（码率）、`scale`（分辨率缩放）、`fps` 三项独立调节。局域网 / 高带宽时拉满 `scale:1.0 bitrate:8M`，手机 4G 时降至 `scale:0.5 bitrate:1M`，随需调整。
- **移动端键盘**：参考 Terminus 的交互设计，修饰键（Ctrl / Shift / Alt / Win / ⌘）支持单独按下，数字行、方向键、F1–F12 常驻工具栏，可发送 Ctrl+B+1 这类 tmux 快捷键组合。
- **会话内聊天**：控制端与被控端通过 WebRTC DataChannel 直连互发文字和文件，不经服务器，被控端收到消息触发系统通知，方便传递链接、截图、配置文件等。
