...
# 会话密码由 agent 每次启动自动生成（8 位数字），无需在此配置。
# 控制端输入设备 ID + 会话密码即可连接。
...
retry:         "5s"              # 断线重连间隔
allow_control: true              # false 时拒绝所有远程控制连接

ca_cert: "deploy/certs/server.crt"   # 使用自签名证书时必填（make cert 生成的路径）
```
...
| `ca_cert` | `--ca-cert` | 自签名 CA 证书路径（使用自签名时必填） | — |
| `allow_control` | `--no-control` | 是否允许被远程控制（`false` 时拒绝所有连接） | `true` |
...
| 字段 | 来源 | 说明 |
|------|------|------|
| **会话密码**（"远程控制"页） | 被控端每次启动自动生成 | 8 位随机数字，显示在"共享本机"页，重启后更新 |
| **设备密钥**（"共享本机"页） | `server.yaml` → `tokens.<device_id>` | 被控端向服务器注册时的 HMAC 鉴权密钥 |
...
- **设备 ID**：9 位随机数字（10⁹ 空间），首次运行自动生成，永久保存在本地。
- **会话密码**：agent 每次启动随机生成 8 位数字（10⁸ 空间），重启后更新，控制端须输入此密码才能接入。

两者缺一不可。设备 ID 不可猜测，会话密码每次启动更新，组合安全性足够高。
